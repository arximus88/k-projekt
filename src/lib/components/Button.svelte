<script>
  ...
  $: {
    classes = `${size} ${width} ${variant} ${icon ? 'with-icon' : ''} ${disabled ? 'disabled' : ''}`;
  }
...
<style>
    ...
    box-shadow: 0px 0px 24px var(--slight-basic),
			inset 0px 0px 0px 1px var(--tertiary-basic);
		border-radius: 6px;
	}
	.outline:hover {
		background: transparent;
		box-shadow: 0px 0px 24px var(--caption-basic),
			inset 0px 0px 0px 2px var(--primary-basic);
		border-radius: 6px;
	}
